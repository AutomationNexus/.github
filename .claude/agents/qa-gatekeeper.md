...
`workspace/` changed, also run `python scripts/validate-governance.py` and
`python -m unittest tests/test_governance_scenarios.py`, and treat any `ERROR`-level
finding or test failure as a blocker. Report pass/fail + blockers only; no edits.
