...
2. If drift is reported: the root copies were hand-edited or the canonical source
   ...
   with `--force` (a force-overwrite of generated drift requires confirmation per
   `CLAUDE.md`'s "Unified authority and confirmation") — a timestamped backup of the
   current root copies is taken automatically before any overwrite.
